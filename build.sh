...
if [ hyperfine ]; then
    ...
    if [[ "$(uname -s)" == "Linux" ]]; then
        cd ./build && hyperfine --prepare 'echo test;sync;echo 3 | sudo tee /proc/sys/vm/drop_caches'  --warmup 10 -N './fib_tail_rust' './fib_tail_llvm' './fib_tail_zig' './fib_tail_odin' './fib_tail_ocaml' './fib_tail_fasm'
    fi
    echo ''
